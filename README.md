...
Được sử dụng làm mô hình so sánh. Kiến trúc gồm: Convolution, MaxPooling, Flatten, Dense, Softmax.
...
* **Metric:** Accuracy
* **Epoch & Batch Size:** (theo `train_model.py`)
...
Giải nén file `train.csv.zip` thu được `train.csv` đặt cùng thư mục với `train_model.py`.

### Bước 2. Huấn luyện mô hình
Nếu muốn huấn luyện lại từ đầu, chạy lệnh sau:
```bash
python train_model.py
```
Sau khi hoàn thành sẽ sinh ra các file: `mnist_mlp.keras`, `mnist_cnn.keras`, `mnist_rf.joblib`.
Đồng thời chương trình sẽ hiển thị các kết quả đánh giá như: Accuracy, Loss, Confusion Matrix, Classification Report, So sánh các mô hình.
...
python app.py
```
Nếu thành công sẽ xuất hiện: `Running on http://127.0.0.1:5000`
...
1. Mở giao diện Web.
2. Dùng chuột vẽ một chữ số từ 0–9.
3. Nhấn nút **Predict**.
4. Hệ thống sẽ: Tiền xử lý ảnh ➔ Chuẩn hóa về kích thước 28×28 ➔ Đưa ảnh vào mô hình MLP ➔ Hiển thị kết quả dự đoán.
...
1. Nhận ảnh từ HTML Canvas dưới dạng Base64.
2. Chuyển sang ảnh Grayscale và đảo màu để phù hợp với MNIST.
3. Cắt vùng chứa chữ số (Bounding Box).
4. Resize về 20×20 và đặt vào khung ảnh 28×28.
5. Chuẩn hóa giá trị pixel về khoảng [0,1].
6. Chuyển thành vector đầu vào của mô hình.
7. Dự đoán bằng mô hình MLP.
...
**Các tiêu chí đánh giá:** Accuracy, Precision, Recall, F1-score, Confusion Matrix.
